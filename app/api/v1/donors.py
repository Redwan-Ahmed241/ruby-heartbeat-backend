...
def upsert_medical_info(
    ...
    med_info = (
        ...
        .filter(MedicalInfo.donor_id == current_user.user_id)
        .first()
    )
    if not med_info:
        med_info = MedicalInfo(donor_id=current_user.user_id)
        db.add(med_info)
...
def get_donation_history(
    ...
    history = (
        ...
        .filter(DonationHistory.donor_id == current_user.user_id)
...
def record_donation_history(
    ...
    current_user: User = Depends(RequireRoles([UserRole.DONOR, UserRole.HOSPITAL_ADMIN, UserRole.SYSTEM_ADMIN])),
    db: Session = Depends(get_db),
):
    """Record a past donation in the donor's history."""
    target_donor_id = current_user.user_id
    ...
    donor = db.query(Donor).filter(Donor.donor_id == target_donor_id).first()
    if donor:
